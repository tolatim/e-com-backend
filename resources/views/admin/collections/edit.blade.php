...
<div class="w-full">
    ...
    <div class="w-full bg-white border border-slate-200 rounded-xl shadow-sm">
        ...
        <div class="px-6 sm:px-8 py-6 border-b border-slate-100">
            <h1 class="text-xl font-semibold text-slate-900">Edit collection</h1>
            <p class="mt-1 text-sm text-slate-500">Update the details for "{{ $collection->name }}".</p>
        </div>

        <form action="{{ route('admin.collections.update', $collection->id) }}" method="POST" enctype="multipart/form-data" class="px-6 sm:px-8 py-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <label for="image"
                    ...
                    <div id="previewWrap" class="{{ $collection->image ? '' : 'hidden' }} flex-shrink-0">
                        <img id="previewImg" src="{{ $collection->image ? asset($collection->image) : '' }}" alt="" class="w-14 h-14 rounded-md object-cover border border-slate-200">
                    </div>

                    <div id="placeholderIcon" class="{{ $collection->image ? 'hidden' : '' }} flex-shrink-0 w-14 h-14 rounded-md bg-white border border-slate-200 flex items-center justify-center text-slate-400 group-hover:text-indigo-500 transition-colors">
                    ...
                    <div class="text-sm">
                        <span class="font-medium text-indigo-600 group-hover:text-indigo-700">Click to {{ $collection->image ? 'replace' : 'upload' }}</span>
            ...
            <div>
                ...
                <div class="flex gap-2 mb-3">
                    ...
                    >
                    <button type="submit" formaction="{{ route('admin.collections.edit', $collection->id) }}" formmethod="GET"
                    ...
                    @forelse ($products as $product)
                        <label class="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 cursor-pointer">
                            <input
                                ...
                                name="products[]"
                                value="{{ $product->id }}"
                                {{ $collection->products->pluck('id')->contains($product->id) ? 'checked' : '' }}
            ...
            <div class="flex items-center justify-between rounded-lg border border-slate-200 px-4 py-3.5">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    ...
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $collection->is_active) ? 'checked' : '' }} class="sr-only peer">
