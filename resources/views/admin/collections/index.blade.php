...
<div class="w-full">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-semibold text-slate-900">Collections</h1>
            <p class="mt-1 text-sm text-slate-500">Manage curated product collections.</p>
        </div>
    ...
    <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        ...
        @if ($collections->isEmpty())
            <div class="flex flex-col items-center justify-center py-16 px-6 text-center">
                ...
                <p class="text-sm font-medium text-slate-700">No collections yet</p>
                <p class="text-sm text-slate-400 mt-1">Create your first collection to start grouping products.</p>
                <a href="{{ route('admin.collections.create') }}"
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-200 bg-slate-50">
                        ...
                        <th class="text-left font-medium text-slate-500 px-6 py-3">Products</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach ($collections as $collection)
                        <tr class="hover:bg-slate-50/60 transition-colors">
                            ...
                            <td class="px-6 py-3 font-medium text-slate-800">{{ $collection->name }}</td>
                            <td class="px-6 py-3 text-slate-400 font-mono text-xs">{{ $collection->slug }}</td>
                            <td class="px-6 py-3">
                                <span class="inline-flex items-center gap-1.5 rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-medium text-indigo-600">
                                    {{ $collection->products->count() }} {{ Str::plural('product', $collection->products->count()) }}
                                </span>
                            </td>
                            <td class="px-6 py-3">
