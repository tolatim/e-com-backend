...
<div class="w-full">
    ...
    <div class="w-full bg-white border border-slate-200 rounded-xl shadow-sm">
        ...
        <div class="px-6 sm:px-8 py-6 border-b border-slate-100">
            <h1 class="text-xl font-semibold text-slate-900">Edit product</h1>
            <p class="mt-1 text-sm text-slate-500">Update the details for "{{ $product->name }}".</p>
        </div>

        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="px-6 sm:px-8 py-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <label for="image"
                    ...
                    <div id="previewWrap" class="{{ $product->image ? '' : 'hidden' }} flex-shrink-0">
                        <img id="previewImg" src="{{ $product->image ? Storage::url($product->image) : '' }}" alt="" class="w-14 h-14 rounded-md object-cover border border-slate-200">
                    </div>

                    <div id="placeholderIcon" class="{{ $product->image ? 'hidden' : '' }} flex-shrink-0 w-14 h-14 rounded-md bg-white border border-slate-200 flex items-center justify-center text-slate-400 group-hover:text-indigo-500 transition-colors">
                    ...
                    <div class="text-sm">
                        <span class="font-medium text-indigo-600 group-hover:text-indigo-700">Click to {{ $product->image ? 'replace' : 'upload' }}</span>
