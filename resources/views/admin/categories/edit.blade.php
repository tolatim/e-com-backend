...
<div class="w-full">
    ...
    <div class="w-full bg-white border border-slate-200 rounded-xl shadow-sm">
            ...
            <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="px-6 sm:px-8 py-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    @enderror
                    <p class="mt-1.5 text-xs text-slate-400">The slug will be generated automatically from this name.</p>
                </div>
                ...
                <div>
                    ...
                    <label for="image"
                        ...
                        <div id="previewWrap" class="{{ $category->image ? '' : 'hidden' }} flex-shrink-0">
                            <img id="previewImg" src="{{ $category->image ? Storage::url($category->image) : '' }}" alt="" class="w-14 h-14 rounded-md object-cover border border-slate-200">
                        </div>

                        <div id="placeholderIcon" class="{{ $category->image ? 'hidden' : '' }} flex-shrink-0 w-14 h-14 rounded-md bg-white border border-slate-200 flex items-center justify-center text-slate-400 group-hover:text-indigo-500 transition-colors">
                        ...
                        <div class="text-sm">
                            <span class="font-medium text-indigo-600 group-hover:text-indigo-700">Click to {{ $category->image ? 'replace' : 'upload' }}</span>
